...
import { serviceVerticalsNav as serviceVerticals, priorityIndustriesNav as priorityIndustries, clustersNav as clusters } from "@/data/navigationData";
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        ...
        <nav aria-label="Main Navigation" className="hidden items-center gap-7 lg:flex">
          {simpleLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className={`group relative inline-block text-sm font-medium transition-all duration-200 hover:scale-105 hover:-translate-y-0.5 after:absolute after:-bottom-1 after:left-0 after:h-0.5 after:w-0 after:bg-white after:transition-all after:duration-300 hover:after:w-full focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-blue rounded ${
          ...
          {megaMenus.map(({ label, href, Menu }) => (
            ...
            >
              <Link
                ...
                aria-haspopup="true"
                className={`relative flex items-center gap-1 text-sm font-medium transition-all duration-200 hover:scale-105 hover:-translate-y-0.5 after:absolute after:-bottom-1 after:left-0 after:h-0.5 after:w-0 after:bg-white after:transition-all after:duration-300 group-hover:after:w-full focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-blue rounded ${
        ...
        <div className="hidden lg:block">
          <Link
            href="/contact-us"
            className="inline-block rounded-md bg-white px-5 py-2.5 text-sm font-semibold text-navy transition-all duration-200 hover:scale-105 hover:-translate-y-0.5 hover:bg-slate-100 hover:shadow-lg active:scale-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-blue"
          >
            Get Consultation
          </Link>
        </div>
        ...
        <motion.button
          whileTap={{ scale: 0.9 }}
          className="text-white transition-transform duration-200 hover:scale-110 lg:hidden focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-blue rounded"
          onClick={() => setOpen((v) => !v)}
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <nav aria-label="Mobile Navigation" className="flex max-h-[80vh] flex-col gap-1 overflow-y-auto px-4 py-4">
              {simpleLinks.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  onClick={() => setOpen(false)}
                  className="rounded-md px-3 py-2 text-sm font-medium text-white/90 transition-all duration-200 hover:translate-x-1 hover:bg-white/10 focus-visible:outline-2 focus-visible:outline-brand-blue"
                >
                  {link.label}
                </Link>
              ))}
              ...
              <div className="rounded-md px-3 py-2">
                <button
                  ...
                  aria-expanded={mobileMenuOpen === "Services"}
                  aria-controls="mobile-services-panel"
                  className="flex w-full items-center justify-between text-sm font-medium text-white/90 focus-visible:outline-2 focus-visible:outline-brand-blue rounded"
                  onClick={() =>
                        ...
                        {serviceVerticals.map((vertical, index) => (
                          <div key={vertical.slug} className={`${index > 0 ? "border-t border-white/10 pt-4" : ""} space-y-2`}>
                            <Link
                              href={`/services/${vertical.slug}`}
                              onClick={() => setOpen(false)}
                              className="inline-block text-sm font-semibold text-white transition-transform duration-200 hover:translate-x-1 focus-visible:outline-2 focus-visible:outline-brand-blue rounded px-1"
                            >
                              {vertical.name}
                            </Link>
                            <div className="mt-1 flex flex-col pl-2 divide-y divide-white/5">
                              {vertical.hubs.map((hub) => (
                                <Link
                                  ...
                                  href={`/services/${vertical.slug}/${hub.slug}`}
                                  onClick={() => setOpen(false)}
                                  className="inline-block py-1.5 text-sm text-white/80 transition-transform duration-200 hover:translate-x-1 hover:text-white focus-visible:outline-2 focus-visible:outline-brand-blue rounded px-1"
                                >
                                  {hub.name}
                                </Link>
                              ))}
              ...
              <div className="rounded-md px-3 py-2">
                <button
                  ...
                  aria-expanded={mobileMenuOpen === "Industries"}
                ...
                <AnimatePresence>
                  {mobileMenuOpen === "Industries" && (
                    <motion.div
                      ...
                      role="region"
                      aria-labelledby="mobile-industries-button"
                      {...accordionCollapse}
                      className="overflow-hidden"
                    >
                      <div className="mt-3 flex flex-col border-l border-white/10 pl-4 divide-y divide-white/5">
                        ...
                        <Link
                          href="/industries"
                          onClick={() => setOpen(false)}
                          className="inline-block pt-2 py-1.5 text-sm font-semibold text-white transition-transform duration-200 hover:translate-x-1 focus-visible:outline-2 focus-visible:outline-brand-blue rounded px-1"
                        >
                          View All Industries
                        </Link>
                      </div>
              ...
              <div className="rounded-md px-3 py-2">
                <button
                  id="mobile-kc-button"
                  aria-expanded={mobileMenuOpen === "Knowledge Center"}
                  aria-controls="mobile-kc-panel"
                  className="flex w-full items-center justify-between text-sm font-medium text-white/90 focus-visible:outline-2 focus-visible:outline-brand-blue rounded"
                  onClick={() =>
                ...
                <AnimatePresence>
                  {mobileMenuOpen === "Knowledge Center" && (
                    <motion.div
                      id="mobile-kc-panel"
                      role="region"
                      aria-labelledby="mobile-kc-button"
                      {...accordionCollapse}
                      className="overflow-hidden"
                    >
                        ...
                        <div className="flex flex-col divide-y divide-white/5 mt-1">
                          {clusters.map((cluster) => (
                            <Link
                              ...
                              href={`/knowledge-center/${cluster.slug}`}
                              onClick={() => setOpen(false)}
                              className="inline-block py-1.5 text-sm text-white/80 transition-transform duration-200 hover:translate-x-1 hover:text-white pl-1 focus-visible:outline-2 focus-visible:outline-brand-blue rounded px-1"
                            >
                              {cluster.name}
                            </Link>
                          ))}
              ...
              <Link
                href="/contact-us"
                onClick={() => setOpen(false)}
                className="mt-2 rounded-md bg-white px-5 py-2.5 text-center text-sm font-semibold text-navy transition-transform duration-200 hover:scale-[1.02] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-blue"
              >
                Get Consultation
              </Link>
            </nav>
